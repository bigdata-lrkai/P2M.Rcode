...
cellType_all <- unique(seurat_obj_merge@meta.data$celltype_singler)
...
for(i in 1:length(Var_top1000_gene)){
  pb$tick()
  num_temp <- t(as.matrix(tapply(1:ncol(P_mtx), seurat_primary$celltype_singler, function(x){
    sum(P_mtx[Var_top1000_gene[i],x]!=0)
  })))
  
  mean_temp <- t(as.matrix(tapply(1:ncol(P_mtx), seurat_primary$celltype_singler, function(x){
...
for(i in 1:length(Var_top1000_gene)){
  ...
  pb2$tick()
  num_temp <- t(as.matrix(tapply(1:ncol(M_mtx), seurat_metastasis$celltype_singler, function(x){
    sum(M_mtx[Var_top1000_gene[i],x]!=0)
  })))
  
  mean_temp <- t(as.matrix(tapply(1:ncol(M_mtx), seurat_metastasis$celltype_singler, function(x){
...
write.csv(gc_df,file.path("output",paste0(GSE_number,"_",Met_site,"_sr_df.csv")), row.names = T)
...
num_celltype_P <- t(as.matrix(table(seurat_primary@meta.data$celltype_singler)))
num_celltype_M <- t(as.matrix(table(seurat_metastasis@meta.data$celltype_singler)))

# library(magrittr)
percent_celltype_P <- t(round(as.matrix(table(seurat_primary@meta.data$celltype_singler))/ncol(seurat_primary),3)) %>% as.data.frame()  #1x7
percent_celltype_M <- t(round(as.matrix(table(seurat_metastasis@meta.data$celltype_singler))/ncol(seurat_metastasis),3)) %>% as.data.frame()  #1x8
...
write.csv(percent_celltype, file.path("output",paste0(GSE_number,"_",Met_site,"_sr_2.csv")), row.names = T)
...
write.csv(ratio_ownType_merge, file.path("output",paste0(GSE_number,"_",Met_site,"_sr_3.csv")), row.names = T)
